...
def detect_pivot(local_highs, current_close, n_bars, config):
    """
    Detect a stricter breakout pivot from a genuine resistance cluster.

    A valid pivot now requires:
    1. At least 2 confirmed swing-high touches.
    2. Touch prices contained within a tight resistance band.
    3. Touches separated by several trading days, so one choppy peak is not
       counted multiple times.
    4. The cluster must span a minimum number of trading days.
    5. No fallback from a single recent swing high. If no valid cluster exists,
       return None ("No Clear Pivot").
    """
    lookback = int(config.get("pivot_lookback_days", 60))
    cluster_pct = float(config.get("pivot_cluster_pct", 2.0)) / 100.0
    min_touches = int(config.get("pivot_min_touches", 2))
    min_touch_sep = int(config.get("pivot_min_touch_separation_days", 5))
    min_cluster_span = int(config.get("pivot_min_cluster_span_days", 10))

    # Recent confirmed swing highs only.
    candidates = sorted(
        [(idx, float(price)) for idx, price in local_highs if idx >= n_bars - lookback],
        key=lambda x: x[0],
    )

    if not candidates:
        return None, "No Clear Pivot: no confirmed swing highs"

    # Ignore resistance levels implausibly far away from the current price.
    candidates = [
        (idx, price)
        for idx, price in candidates
        if current_close * 0.85 <= price <= current_close * 1.12
    ]

    if len(candidates) < min_touches:
        return None, f"No Clear Pivot: fewer than {min_touches} nearby swing highs"

    # Build candidate resistance clusters.
    #
    # Important: cluster_pct applies to the TOTAL width of the cluster,
    # not ±cluster_pct around an anchor. E.g. with 2%, the highest and lowest
    # touch in the cluster must be within roughly 2% of each other.
    valid_clusters = []

    for start in range(len(candidates)):
        for end in range(start + 1, len(candidates)):
            raw_cluster = candidates[start:end + 1]

            prices = [p for _, p in raw_cluster]
            ...
            width_pct = cluster_high / cluster_low - 1.0
            if width_pct > cluster_pct:
                continue

            # De-duplicate nearby swing highs. Two highs only a day or two apart
            # are usually one topping attempt, not two independent "touches".
            touches = []
            for idx, price in raw_cluster:
                if not touches or idx - touches[-1][0] >= min_touch_sep:
                    touches.append((idx, price))
                else:
                    # If two highs are too close in time, retain the higher high
                    # as the representative touch.
                    if price > touches[-1][1]:
                        touches[-1] = (idx, price)

            if len(touches) < min_touches:
                continue

            span_days = touches[-1][0] - touches[0][0]
            if span_days < min_cluster_span:
                continue

            touch_prices = [p for _, p in touches]
            touch_low = min(touch_prices)
            touch_high = max(touch_prices)
            touch_width_pct = (touch_high / touch_low - 1.0) * 100.0

            valid_clusters.append({
                "touches": touches,
                "touch_count": len(touches),
                "span_days": span_days,
                "width_pct": touch_width_pct,
                "latest_idx": touches[-1][0],
                "pivot": touch_high,
            })
    ...
    if not valid_clusters:
        return (
            None,
            "No Clear Pivot: no resistance cluster meeting "
            f"{min_touches}+ touches / {min_touch_sep}D separation / "
            f"{min_cluster_span}D span / {cluster_pct * 100:.1f}% width",
        )

    # Prefer:
    #   1. More independent touches
    #   2. Longer-established resistance
    #   3. More recent cluster
    #   4. Tighter price band
    best = max(
        valid_clusters,
        key=lambda c: (
            c["touch_count"],
    ...
    pivot = best["pivot"]
    reason = (
        f"Resistance cluster: {best['touch_count']} touches / "
        f"{best['span_days']}D span / {best['width_pct']:.1f}% width"
    )
